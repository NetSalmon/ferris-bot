...
impl BatchAPI for Client {
    async fn send(&mut self, content: &Sender<String>, reason: &Sender<String>, request: &Request) -> Result<Response, AppError> {
        ...
        let text = self.client.post(self.base_url.clone())
        ...
        let (got_content, got_reason) = result.choices
            .iter()
            .fold((String::new(), String::new()), |(mut temp_content, mut temp_reason), choice | {
